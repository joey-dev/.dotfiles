...
local function search_and_display_groups()
    ...
    attach_mappings = function(prompt_bufnr, map)
      map('i', '<CR>', function()
        ...
        elseif selected == "all unit" then
          command = "./test_runner unit"
        elseif selected == "all integration" then
          command = "./test_runner integration"
        elseif selected == "all api" then
          command = "./test_runner api"
        else
          local test_group, word = selected:match("^(%S+) (%S+)$")
          command = string.format("./test_runner debug %s --group %s", test_group, word)
        end

        vim.cmd(string.format(":below 10split | :terminal cd %s && %s", git_root, command))
      end)
      return true
    end,
  }):find()
...
vim.keymap.set('n', '<Leader>at', search_and_display_groups, { noremap = true, silent = true })
